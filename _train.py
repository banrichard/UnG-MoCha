...
from utils.utils import anneal_fn, get_enc_len, load_data, get_linear_schedule_with_warmup, \
...
def train(model, optimizer, scheduler, data_type, data_loader, device, config, epoch, logger=None, writer=None):
    ...
    if config["reg_loss"] == "MAE":
        ...
        reg_crit = lambda pred, target: F.mse_loss(F.relu(pred), target) + F.l1_loss(F.relu(pred), target)
    ...
    if config["bp_loss"] == "MAE":
        ...
        bp_crit = lambda pred, target, neg_slp: F.mse_loss(F.leaky_relu(pred, neg_slp), target) + F.l1_loss(
    ...
    for batch_id, batch in enumerate(data_loader):
        ids, pattern, pattern_len, pattern_e_len, graph, graph_len, graph_e_len, counts, p_e_max, g_e_max, p_index, g_index = batch
        cnt = counts.shape[0]
        total_cnt += cnt
        pattern.to(device)
        graph.to(device)
        pattern_len, pattern_e_len, graph_len, graph_e_len, counts, p_e_max, g_e_max, p_index, g_index = \
            pattern_len.to(device), pattern_e_len.to(device), \
                graph_len.to(device), graph_e_len.to(device), counts.to(device), \
                p_e_max.to(device), g_e_max.to(device), p_index.to(device), g_index.to(device)
        s = time.time()
        pred, filmreg = model(pattern, pattern_len, pattern_e_len, graph, graph_len, graph_e_len, p_e_max, g_e_max,
                              p_index, g_index)
        ...
        if isinstance(config["bp_loss_slp"], (int, float)):
            ...
            neg_slp = anneal_fn(bp_loss_slp, batch_id + epoch * epoch_step, T=total_step // 4, lambda0=float(l0),
                                lambda1=float(l1))
        bp_loss = bp_crit(pred, counts, neg_slp) + train_config["weight_decay_film"] * filmreg
...
def evaluate(model, data_type, data_loader, device, config, epoch, logger=None, writer=None):
    ...
    with torch.no_grad():
        for batch_id, batch in enumerate(data_loader):
            ...
            pattern.to(device)
            graph.to(device)
            pattern_len, graph_len, counts, p_e_max, g_e_max, pattern_e_len, graph_e_len, p_index, g_index = pattern_len.to(
                device), graph_len.to(device), counts.to(device), p_e_max.to(device), g_e_max.to(device), \
                pattern_e_len.to(device), graph_e_len.to(device), p_index.to(device), g_index.to(device)
            st = time.time()
            pred, filmreg = model(pattern, pattern_len, pattern_e_len, graph, graph_len, graph_e_len, p_e_max, g_e_max,
                                  p_index, g_index)
            et = time.time()
