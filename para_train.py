...
def train(model, optimizer, scheduler, data_type, data_loader, device, config, epoch, graph, logger=None, writer=None,
    ...
    for i, batch in enumerate(data_loader):
        ...
        card = card.cuda()
        var = var.cuda()
        if config['predict_net'].startswith("Film") or config['predict_net'].startswith("CCA") or config[
            'predict_net'].startswith("Wasserstein"):
            pred, pred_var, filmreg = model(motif.x, motif.edge_index, motif.edge_attr, graph)
            ...
            pred, pred_var = model(motif.x, motif.edge_index, motif.edge_attr, graph)
            # y_pred = val_to_distribution(pred.detach().cpu(), pred_var.detach().cpu()).cuda(0)
            # y_pred = torch.tensor(y_pred.view(-1, 1), requires_grad=True)
            bp_loss = (1 - config['weight_decay_var']) * bp_crit(pred, card) + config['weight_decay_var'] * bp_crit(
                pred_var, var)
        ...
        if logger and ((i / config['batch_size']) % config["print_every"] == 0 or i == epoch_step - 1):
            logger.info(
                "epoch: {:0>3d}/{:0>3d}\tdata_type: {:<5s}\tbatch: {:0>5d}/{:0>5d}\treg loss: {:.5f}\tbp loss: {:.5f}\t".format(
...
if __name__ == "__main__":
    ...
    if train_config["gpu_id"] != -1:
        ...
        print("Now GPU:%s is working!" % torch.cuda.current_device())
        # load data
